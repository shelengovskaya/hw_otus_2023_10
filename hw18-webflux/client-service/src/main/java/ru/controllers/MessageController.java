...
public class MessageController {
    ...
    public void getMessage(@DestinationVariable String roomId, Message message) {
        ...
        if (roomId.equals(MAGIC_ROOM)) {
            throw new RuntimeException("You can't write messages in this room.");
        }
    ...
    private Flux<Message> getMessagesByRoomId(long roomId) {
        String uri = (Long.parseLong(MAGIC_ROOM) == roomId) ? "/msg/all" : String.format("/msg/%s", roomId);
        return datastoreClient
                .get()
                .uri(uri)
